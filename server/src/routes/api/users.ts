...
// 	if (!username || !email || !password || !passwordCheck) return res.status(400).json({ err: "Missing field" });

// 	// [S:01] Checking if the password and password Check matach
// 	if (password != passwordCheck) return res.status(400).json({ err: "Password does't match" });
...
// 	if (!email || !password) return res.status(400).json({ err: "Missing field" });
